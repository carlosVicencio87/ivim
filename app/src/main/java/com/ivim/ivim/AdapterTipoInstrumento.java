...
public class AdapterTipoInstrumento extends ArrayAdapter<String> {
    private Activity activity;
    private ArrayList data;
    public Resources res;
    SpinnerModel spinnerModel=null;
    LayoutInflater inflater;
    public AdapterTipoInstrumento(Mapa activitySpinner, int textViewResourceId, ArrayList objects, Resources resLocal)
    {
    ...
    public View getCustomView(int position, View convertView, ViewGroup parent)
    {
        ...
        View row = inflater.inflate(R.layout.lista_tipo_instrumento,parent,false);
        spinnerModel = null;
        spinnerModel= (SpinnerModel) data.get(position);
        TextView label = (TextView) row.findViewById(R.id.listaInstrumento);
        //ImageView iconoTipo = (ImageView) row.findViewById(R.id.imagenSpinner);
        if (position==0)
        {
            label.setText("Tipo de instrumento");
        }
        else
        {
